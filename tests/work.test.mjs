...
test('the private project shows a chip, the public ones a README link', () => {
  const cards = cardsOf(pages.gallery);
  PROJECTS.forEach((p, i) => {
    const external = openTags(cards[i], 'a').filter((a) => a.includes('target="_blank"'));
    if (p.private) {
      assert.ok(textOf(cards[i]).includes(WORK.privateRepo), 'BIBO says the repository is private');
      assert.equal(external.length, 0, 'a private project links nothing out');
      assert.doesNotMatch(cards[i], /github\.com/, 'no repository link anywhere on the BIBO card');
    } else {
...
test('the private case page shows the scope note and links no repository', () => {
  const p = projectById('bibo');
  const body = textOf(pages.bibo);
  assert.ok(body.includes(WORK.privateRepo), 'lock label');
  assert.ok(body.includes(p.scope), 'scope note');
  assert.doesNotMatch(markup(pages.bibo), /github\.com/, 'no github link on a private case page');
});
...
test('the case pages walk in a loop and offer both ways out', () => {
  ...
  assert.ok(textOf(pages.bibo).includes(`BIBO · 1 of ${total}`), 'subtitle counts the position');
  assert.ok(textOf(pages.bibo).includes(`1 / ${total}`), 'footer counter');
  assert.ok(tagWithHref(pages.bibo, '/work'), '← All work');
  assert.ok(tagWithHref(pages.bibo, '/'), 'close goes home');
  ...
  // bibo is first: previous wraps to amm (last), next is loop.
  assert.match(nav('bibo').prev, /href="\/work\/amm"/);
  assert.match(nav('bibo').next, /href="\/work\/loop"/);
  // amm is last: next wraps back to bibo.
  assert.match(nav('amm').next, /href="\/work\/bibo"/);
