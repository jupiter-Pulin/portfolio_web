...
test('a project already in scope is not switched to again', () => {
  assert.deepEqual(route('bibo', 'bibo'), {
    key: 'overview',
    scopeId: 'bibo',
    scopeChanged: false,
  });
  // Naming a different project from inside a scope does move.
  assert.deepEqual(route('and the amm?', 'bibo'), {
    key: 'overview',
    scopeId: 'amm',
    scopeChanged: true,
  });
...
test('chips are the five global questions, or four scoped ones plus a way back', () => {
  ...
  const scoped = chipsFor('BIBO');
  assert.deepEqual(
    scoped.map((c) => c.key),
    ['decision', 'stack', 'status', 'code', 'all'],
  );
  assert.equal(scoped[0].label, 'Hardest decision in BIBO?');
...
test('the drawer greets once a page, and only announces a scope that changed', () => {
  ...
  const scopedOpen = openIntro({ scopeId: 'bibo', previousScopeId: null, greeted: false });
  assert.equal(scopedOpen.length, 2);
  assert.equal(answerText(scopedOpen[1]), scopedNotice('BIBO'));

  assert.deepEqual(openIntro({ scopeId: 'bibo', previousScopeId: 'bibo', greeted: true }), []);
  assert.equal(
    openIntro({ scopeId: 'loop', previousScopeId: 'bibo', greeted: true }).length,
    1,
    'a different project is announced',
  );
  assert.deepEqual(
    openIntro({ scopeId: null, previousScopeId: 'bibo', greeted: true }),
    [],
    'dropping the scope says nothing',
  );
});

test('the payments answer is a report on three projects with mail and copy', () => {
  ...
  assert.deepEqual(
    ...
    ['Open BIBO ↗', 'Open AMM DEX ↗', 'Open Loop Conductor ↗'],
  );
  assert.equal(card.items[0].runs[0].v, 'BIBO', 'each line leads with the project name');
  assert.ok(card.note.startsWith('Scope note: BIBO is team-built'), 'the scope note stays');
...
test('the code answer links one repo per public project, and marks the private one', () => {
  ...
  assert.deepEqual(rows[0].right, { t: 'muted', text: 'private · team-built' });
...
test('a scoped code answer shows only that project', () => {
  ...
  // A private project in scope is still a private project.
  const bibo = answerBlocks('code', 'bibo').find((b) => b.kind === 'rows').rows;
  assert.deepEqual(bibo, [{ label: 'BIBO', right: { t: 'muted', text: GUIDE.code.private } }]);
});
...
test('decision, stack and status quote the project qa and nothing else', () => {
  ...
  assert.equal(scopedHeading('BIBO', 'decision'), 'BIBO · one decision, in detail');
});
...
test('"All questions" is a plain line, and the scope is cleared by the caller', () => {
  assert.deepEqual(answerBlocks('all', 'bibo'), [
